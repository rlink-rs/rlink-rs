...
use rlink_core::cluster::TaskResourceInfo;
...
impl KubernetesResourceManager {
    pub fn new(context: Arc<Args>) -> Self {
...
impl TResourceManager for KubernetesResourceManager {
    fn prepare(&mut self, _context: &Args, job_descriptor: &ClusterDescriptor) {
        self.cluster_descriptor = Some(job_descriptor.clone());
    }

    async fn worker_allocate(
        &self,
        _env: Arc<ApplicationEnv>,
